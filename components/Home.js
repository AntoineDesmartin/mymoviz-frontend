...
function Home() {
  ...
  const [moviesDat,setMoviesDat]=useState([])
  ...
  useEffect(() => {
    fetch("http://localhost:3000/movie").then(res => res.json()).then(data =>{
  ...
  // console.log('moviesDat :>> ', moviesDat);

  const movies = moviesDat.map((data, i) => {
